...
public class DragthisCubeP2 : MonoBehaviour {
  ...
  private void OnMouseDown() {
    if (Puzzle2DragManager._drag.CurrentDragCube == null) {
      Puzzle2DragManager._drag.CurrentDragCube = this;
  ...
  private void OnMouseDrag() {
    ...
    //if (Puzzle2DragManager._drag.CurrentDragCube != this) {
    //return;
    //}
    // Debug.Log("drag" + this.transform.gameObject.name);
    //if (canMove != false) { return; }

    if (Puzzle2DragManager._drag.isDraging == true) {
      return;
    }
    ...
    if (Mathf.Abs(Vector3.Distance(objectPosition, lastCubePostion)) >= stepLength) {
      ...
      // Debug.Log(Puzzle2DragManager._drag.CanthisGridMove(curx, curY, "Up"));
      ...
      if (Mathf.Abs(direction.x) >= Mathf.Abs(direction.y)) {
        ...
        if (direction.x > 0) {
          ...
          if (Puzzle2DragManager._drag.CanthisGridMove(curx, curY, "Right")) {
            Debug.Log("向右滑");
            soundManager.PlayEndingDrag();
            // Puzzle2DragManager._drag.cellArray[curx, curY] = 0;
          ...
          if (Puzzle2DragManager._drag.CanthisGridMove(curx, curY, "Left")) {
            // Puzzle2DragManager._drag.cellArray[curx, curY] = 0;
            Debug.Log("向左滑");
            soundManager.PlayEndingDrag();
        ...
        if (Mathf.Abs(direction.y) > Mathf.Abs(direction.x)) {
          if (direction.y > 0) {
            if (Puzzle2DragManager._drag.CanthisGridMove(curx, curY, "Up")) {
              // Puzzle2DragManager._drag.cellArray[curx, curY] = 0;
              Debug.Log("向上滑");
              soundManager.PlayEndingDrag();
            ...
            if (Puzzle2DragManager._drag.CanthisGridMove(curx, curY, "Down")) {
              Debug.Log("向下滑");
              soundManager.PlayEndingDrag();
        ...
        // if (direction.y > 0 && Puzzle2DragManager._drag.CanthisGridMove(curx, curY, "Up")) {
        //   // Puzzle2DragManager._drag.cellArray[curx, curY] = 0;
        // Debug.Log("向上滑");

        //   // StartCoroutine(MoveStone(Vector3.up, Mathf.Sign(direction.y), false));
        // }
        // else if (direction.y < 0 && Puzzle2DragManager._drag.CanthisGridMove(curx, curY, "Down")) {
        //   Debug.Log("向下滑");
  ...
  private void OnMouseUp() {
    ...
    Puzzle2DragManager._drag.CurrentDragCube = null;
  }

  IEnumerator MoveStone(Vector3 direction, float dir, bool xory) {
    Puzzle2DragManager._drag.isDraging = true;
    ...
    Puzzle2DragManager._drag.isDraging = false;
    ...
    // Puzzle2DragManager._drag.cellArray[curx, curY] = 1;
  }
}
